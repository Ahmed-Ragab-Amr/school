<div class="wg-box">
    <div class="title-box">
        ...
        <div class="body-text">ابحث عن الامن والسلامه بالإسم أو رقم الهويه </div>
        ...
        <ul class="flex flex-column">
            ...
            <li class="product-item gap14">
                <div class="image no-bg">
                    <img src="{{ asset("uploads/$safty->image_path") }}" alt="">
                </div>
                <div class="flex items-center justify-between gap20 flex-grow">
                    <div class="name">
                        <a href="{{ route('profile.user' , $safty->id) }}" class="body-title-2">
                        {{ $safty->name }}</a>
                    </div>
                    <div class="body-text" style="transform: translateX(-9%);">{{ $safty->national_id }}</div>
                    <div class="body-text"style="transform: translateX(5%);">{{ $safty->password }}</div>
                    ...
                    <div class="list-icon-function" >
                        <div  class="">
                            <a href="{{ route('profile.user' , $safty->id) }}"><i  style="color:#3b82f6; font-size:15px" class="fas fa-eye eye-icon"></i></a>
                        </div>
                        <div class="item edit">

                            <a href="{{ route('user.edit' , $safty->id) }}"><i style="color: green;"  class="icon-edit-3"></i></a>
                        </div>
                        <div class="">
                            <form action="{{ route('user.delete' , $safty->id) }}" method="post">
                                @csrf
                                @method('delete')
